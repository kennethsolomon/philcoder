...
class ReadingItemManager{
                    ...
                    if(value){      
                      ...
                      for(let i=0; i<this.items.length; i++){
                        if(this.items[i].itemid == itemid){
                            if(this.items[i].type == 'qa'){this.items[i].detachListeners()};
    ...
    setupEventHandlerListener(){
        ...
        $('#carditem-con-id-'+this.itemid).find('.btn-sort-items').click((e)=>{
            ...
            if($(e.currentTarget).text() === 'Sort items'){
                ...
                $('#carditemid_'+this.itemid).find('div.items-container > ul').css({'background-color':'rgb(230, 226, 226)', 'display':'block'});
                $('#carditemid_'+this.itemid).find('div.items-container > ul > li').css({'background-image':'url(assets/icons/baseline-dehaze-black-18/2x/baseline_dehaze_black_18dp.png)','cursor':'move'});
                $('#carditemid_'+this.itemid).find('div.items-container > ul > li .editable').css({'cursor':'move'});
                $(e.currentTarget).text('Disable sort');
                $('#carditemid_'+this.itemid).find('div.sidebar ul li span').css({'cursor':'not-allowed'});
            }else{
                $('#carditemid_'+this.itemid).find('div.items-container > ul').sortable('disable');//disable sortable 
                $('#carditemid_'+this.itemid).find('div.items-container > ul').css({'background-color':'white', 'display':'flex'});
                $('#carditemid_'+this.itemid).find('div.items-container > ul > li').css({'background-image':'none','cursor':'text'});
                $('#carditemid_'+this.itemid).find('div.items-container > ul > li .editable').css({'cursor':'text'});
                $(e.currentTarget).text('Sort items');
                $('#carditemid_'+this.itemid).find('div.sidebar > ul > li > span').css({'cursor':'pointer'});
            }        
        });
    ...
    confirmDisableReadingitemSortable(){
        ...
        if(parseInt(itemslength) <= 0){
            $('#carditemid_'+this.itemid).find('div.items-container ul').sortable('disable');//disable sortable 
            $('#carditemid_'+this.itemid).find('div.items-container ul').css({'background-color':'white'});
            $('#carditemid_'+this.itemid).find('div.items-container ul li').css({'background-image':'none','cursor':'text'});
            $('#carditemid_'+this.itemid).find('div.items-container ul li .editable').css({'cursor':'text'});
    ...
    deleteAllItem(){
        ...
        var updates = {}, items = $('#carditemid_'+this.itemid).find('.items-container ul').children();
    ...
    updatereadingitemslist(){
        ...
        var updates = {}, itemsidlist = [], items = $('#carditemid_'+this.itemid).find('div.items-container ul').children();//get all items
    ...
    saveItem(itemtype){
        ...
        if(itemtype == 'qa'){
            iteminfo = {
                ...
                correct_answers: [{isDeleted: false, textcontent: '', message: ''}],
                question_options: [{isDeleted: false, textcontent: '', message: ''}]
            };
        }else{
            iteminfo = {
                carditemid: 'carditemid_' + this.itemid,
