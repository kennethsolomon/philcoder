...
class QuizItemManager {
  ...
  setupUploadImageDialog() {
    ...
    dialog
      .querySelector("#dialog-" + this.itemid + " .close")
  ...
  saveItemValue(fieldType) {
    ...
    firebase
      ...
      .once("value", snap => {
        firebase
          .database()
          .ref(Ref + (snap.val() ? snap.val().length : 0))
          .set({ isDeleted: false })
          .then(() => {})
          .catch(err => {
            console.log(err);
