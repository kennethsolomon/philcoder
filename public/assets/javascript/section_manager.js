...
class SectionManager {
        ...
        $('#sortcard'+this.sectionid).click((e)=>{
          ...
          if(c.firstElementChild.textContent === 'sort'){
            ...
            for(let i=0; i < cards.length; i++){
                $(cards[i]).css({'cursor':'move'})//sets all cards height to max-content
            ...
            .css({'background-color':'rgb(230, 226, 226)','display':'block'})
            ...
            for(let i=0; i < cards.length; i++){
                $(cards[i]).css({'cursor':'default'})//sets all cards cursor to default
                .find('.card-view-but') 
            ...
            $('#sectionid_'+this.sectionid).find('.create-card-cont')
            .css({'background-color':'white','display':'flex'})
    ...
    setcardsView(viewtype){
        ...
        if(viewtype == 'list'){
            for(let i=0; i < cards.length; i++){
                ...
                $(cards[i]).css({'height':'80px'});//sets all cards height to 80px
                $(cards[i]).find('.elements-items-container').css({'display':'none'});//Hides all cards contents
                $(cards[i]).find('.card-panel').css({'display':'block'});//Displays the cards panel for listview
            ...
            for(let i=0; i < cards.length; i++){
                $(cards[i]).css({'height':'auto'});//sets all cards height to auto
                $(cards[i]).find('.elements-items-container').css({'display':'block'});//Displays all cards contents
                $(cards[i]).find('.card-panel').css({'display':'none'});//Hides the cards panel
    ...
    updateSectionCardList(){
        ...
        var updates = {}, cardidlist = [], cards = $('#sectionid_'+this.sectionid).find('.create-card-cont').children();//get all sections
    ...
    deleteSection(){    
        firebase.database().ref('section/' + this.theUser.uid + '/classid_' + this.classid + '/sectionid_' + this.sectionid).update({'isDeleted':true})
        .then(() => {   
    ...
    showUndoSnackBar(){
        ...
        var handler = (event)=> {
            firebase.database().ref('section/' + this.theUser.uid + '/classid_' + this.classid + '/sectionid_' + this.sectionid ).update({'isDeleted':false})
            .then(() => {   
            }).catch((err)=>{
                console.log(err);  
            }); 
